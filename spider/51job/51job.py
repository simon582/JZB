...
def work(list_url):
    ...
    info_list = hxs.xpath('//tr[@class="tr0"]')
    if len(info_list) == 0:
        return False
    for info in info_list:
        try:
            prod = {}
            prod['source'] = '51job'
            prod['url'] = info.xpath('.//td[@class="td1"]/a/@href')[0].extract()
            print 'url: ' + prod['url']
            prod['title'] = ''
            for part in info.xpath('.//td[@class="td1"]/a/text()|.//td[@class="td1"]/a/font/text()'):
                prod['title'] += part.extract().strip()
            print 'title: ' + prod['title']
            prod['vertical'] = False
            print 'vertical: ' + str(prod['vertical'])
            prod['company'] = info.xpath('.//td[@class="td2"]/a/text()')[0].extract()
...
if __name__ == "__main__":
    start_url = "http://search.51job.com/jobsearch/search_result.php?fromJs=1&jobarea=010000%2C00&district=000000&funtype=0000&industrytype=00&issuedate=9&providesalary=99&keyword=%E5%85%BC%E8%81%8C&keywordtype=1&curr_page=#CUR_PAGE#&lang=c&stype=2&postchannel=0000&workyear=99&cotype=99&degreefrom=99&jobterm=01&companysize=99&lonlat=0%2C0&radius=-1&ord_field=0&list_type=0&fromType=14"
    page = 1
